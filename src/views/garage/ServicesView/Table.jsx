...
const ServicesTable = () => {
   ...
   const { singularName, pluralName, services, showServices, deleteServices, setTextBtnSumbit, setFormTitle } = useServiceContext();
      ...
      try {
         mySwal.fire(QuestionAlertConfig(`Estas seguro de eliminar a "${name}"`)).then(async (result) => {
            if (result.isConfirmed) {
               setLoadingAction(true);
               const axiosResponse = await deleteServices(id);
               setLoadingAction(false);
   ...
   const columns = ["Folio", "N° Unidad", "Contacto", "Pre Diagnostico", "Estatus", "Acciones"];
   const data = [];
   const chargerData = async () => {
      try {
         ...
         await services.map((obj) => {
            ...
            register.push(<ButtonsAction id={obj.id} name={obj.service} />);
            data.push(register);
         });
         setLoading(false);
      } catch (error) {
