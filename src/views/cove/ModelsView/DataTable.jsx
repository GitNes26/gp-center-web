...
const ModelDT = () => {
   ...
   const globalFilterFields = ["model", "description"];
   ...
   const columns = [
      { field: "brand", header: "Marca", sortable: true, functionEdit: null, body: ImagePreviewBodyTemplate, filter: true, filterField: null },
      { field: "model", header: "Modelo", sortable: true, functionEdit: null, body: ModelBodyTemplate, filter: true, filterField: null }
   ];
   ...
   const handleClickAdd = () => {
      try {
         resetModel();
         // model.role = "Selecciona una opción...";
         resetFormData();
         setOpenDialog(true);
   ...
   const handleClickDeleteMultipleContinue = async (selectedData) => {
      try {
         ...
         if (selectedData.length === 1) msg += `el modelo: ${selectedData[0].model}?`;
         else if (selectedData.length > 1) msg += `los siguientes modelos: ${selectedData.map((d) => d.model)}?`;
   ...
   const handleClickDisEnable = async (id, name, active) => {
      try {
         let axiosResponse;
         setTimeout(async () => {
            axiosResponse = await disEnableModel(id, !active);
            Toast.Customizable(axiosResponse.alert_text, axiosResponse.alert_icon);
         }, 500);
      } catch (error) {
         console.log(error);
   ...
   const formatData = async () => {
      try {
         ...
         await models.map((obj, index) => {
            // console.log(obj);
            let register = obj;
            register.key = index + 1;
            register.actions = <ButtonsAction id={obj.id} name={obj.model} active={obj.active} />;
            data.push(register);
         });
