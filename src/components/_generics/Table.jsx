...
const GenericTable = () => {
   ...
   const { singularName, pluralName, generics, showGeneric, deleteGeneric, setTextBtnSumbit, setFormTitle } = useGenericContext();
   ...
   const chargerData = async () => {
      try {
         ...
         await generics.map((obj) => {
            // console.log(obj);
            const register = [];
            register.push(<Typography textAlign={"center"}>{obj.generic}</Typography>);
            register.push(<Typography textAlign={"center"}>{obj.description}</Typography>);
            register.push(<ButtonsAction id={obj.id} name={obj.generic} />);
            data.push(register);
         });
         setLoading(false);
      } catch (error) {
