...
const checkAddInitialState =
	localStorage.getItem("checkAdd") == "true" ? true : false || false;
...
const FormComponent = ({ dataCities, dataColonies, dataLevels }) => {
	...
	const validationSchema = Yup.object().shape({
		...
		loc_for: Yup.string().required(
			"Indica si la esculea esá dentro o fuera del municipio de Gomez Palacio"
		),
	...
	const selectedValues = useMemo(() => dataLevels.filter((v) => v.selected), [
		dataLevels,
	]);
				...
				<Typography variant='h2' mb={3}>
					...
					<FormControlLabel
						...
						control={
							<Switch
								checked={checkAdd}
								onChange={(e) => handleChangeCheckAdd(e)}
							/>
						}
                           ...
                           {errors.phone}
							...
							<Grid xs={12} md={6} sx={{ mb: 1 }}>
								<FormControl fullWidth sx={{ alignItems: "center" }}>
									<FormLabel id='loc_for-label'>
										Ubicacion de escuela
									</FormLabel>
									<RadioGroup
										row
